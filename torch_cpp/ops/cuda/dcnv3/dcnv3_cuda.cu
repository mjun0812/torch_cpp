...
at::Tensor dcnv3_cuda_forward(
  ...
  auto output = at::zeros(
      {batch, height_out, width_out, group * group_channels}, input.options());
  ...
  auto per_offset_size = height_out * width_out * group *
                         (kernel_h * kernel_w - remove_center) * 2;
  auto per_mask_size =
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
        input.scalar_type(), "ms_deform_attn_forward_cuda", ([&] {
          dcnv3_im2col_cuda(
              ...
              columns.data<scalar_t>(), kernel_h, kernel_w, stride_h, stride_w,
              pad_h, pad_w, dilation_h, dilation_w, group, group_channels,
              batch_n, height_in, width_in, height_out, width_out, offset_scale,
              remove_center);
        }));
  }
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
        input.scalar_type(), "ms_deform_attn_backward_cuda", ([&] {
          dcnv3_col2im_cuda(
              ...
              dilation_w, group, group_channels, batch_n, height_in, width_in,
              height_out, width_out, offset_scale, remove_center,
              grad_input.data<opmath_t>() + n * im2col_step_ * per_input_size,
              grad_offset.data<opmath_t>() + n * im2col_step_ * per_offset_size,
              grad_mask.data<opmath_t>() + n * im2col_step_ * per_mask_size);
        }));
  }

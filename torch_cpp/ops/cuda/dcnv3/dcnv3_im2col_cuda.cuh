      ...
      for (int j = 0; j < kernel_h; ++j) {
        ...
        if (i != center_w || j != center_h || !remove_center) {
          ...
          if (loc_h > -1 && loc_w > -1 && loc_h < height_in &&
              loc_w < width_in) {
            col += dcnv3_im2col_bilinear(data_im_ptr, height_in, width_in,
                                         group, group_channels, loc_h, loc_w,
                                         g_col, c_col) *
                   weight;
          }
          data_weight_ptr += 1;
      ...
      for (int j = 0; j < kernel_h; ++j) {
        ...
        if (i != center_w || j != center_h || !remove_center) {
          ...
          *(cache_grad_mask + threadIdx.x) = 0;
          if (loc_h > -1 && loc_w > -1 && loc_h < height_in &&
              loc_w < width_in) {
            dcnv3_col2im_bilinear(data_im_ptr, height_in, width_in, group,
                                  group_channels, loc_h, loc_w, g_col, c_col,
                                  offset_scale, top_grad, weight, grad_im_ptr,
...
template <typename scalar_t>
__global__ void dcnv3_col2im_gpu_kernel_gm(const int num_kernels,
                                           const scalar_t* grad_col,
                                           const scalar_t* data_im,
                                           const scalar_t* data_offset,
                                           const scalar_t* data_mask,
    ...
    for (int i = 0; i < kernel_w; ++i) {
      for (int j = 0; j < kernel_h; ++j) {
        ...
        if (i != center_w || j != center_h || !remove_center) {
          ...
          if (loc_h > -1 && loc_w > -1 && loc_h < height_in &&
              loc_w < width_in) {
            dcnv3_col2im_bilinear_gm(data_im_ptr, height_in, width_in, group,
                                     group_channels, loc_h, loc_w, g_col, c_col,
                                     offset_scale, top_grad, weight,
...
void dcnv3_im2col_cuda(cudaStream_t stream,
  ...
  dcnv3_im2col_gpu_kernel<scalar_t>
      <<<GET_BLOCKS(num_actual_kernels, num_threads), num_threads, 0, stream>>>(
          num_kernels, data_im, data_offset, data_mask, data_col, kernel_h,
          kernel_w, stride_h, stride_w, pad_h, pad_w, dilation_h, dilation_w,
          group, group_channels, height_in, width_in, height_out, width_out,
...
template <typename scalar_t>
void dcnv3_col2im_cuda(cudaStream_t stream,
                       const scalar_t* grad_col,
                       const scalar_t* data_im,
                       const scalar_t* data_offset,
                       const scalar_t* data_mask,
      ...
      dcnv3_col2im_gpu_kernel_shm_reduce_v2_multi_blocks<scalar_t>
          <<<GET_BLOCKS(num_actual_kernels, num_threads), num_threads,
             num_threads * 3 * sizeof(opmath_t), stream>>>(
              num_kernels, grad_col, data_im, data_offset, data_mask, kernel_h,
              kernel_w, stride_h, stride_w, pad_h, pad_w, dilation_h,
              dilation_w, group, group_channels, height_in, width_in,
              height_out, width_out, offset_scale, remove_center, grad_im,
      ...
      dcnv3_col2im_gpu_kernel_gm<scalar_t>
          <<<GET_BLOCKS(num_actual_kernels, num_threads), num_threads, 0,
             stream>>>(num_kernels, grad_col, data_im, data_offset, data_mask,
                       ...
                       remove_center, grad_im, grad_offset, grad_mask);
    }
  } else {
        ...
        dcnv3_col2im_gpu_kernel_shm_blocksize_aware_reduce_v1<scalar_t, 1>
            <<<GET_BLOCKS(num_actual_kernels, num_threads), num_threads, 0,
               stream>>>(num_kernels, grad_col, data_im, data_offset, data_mask,
                         ...
                         offset_scale, remove_center, grad_im, grad_offset,
                         grad_mask);
        break;
      case 2:

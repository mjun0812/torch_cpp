...
template <typename scalar_t,
          ...
          int K,
          bool softmax>
__global__ void backward_kernel_dcn(const scalar_t* p_value,
...
__global__ void backward_kernel_dcn_warp_primitive(
  ...
  opmath_t* cache_g_mask_before_softmax =
      (opmath_t*)((opmath_t*)(_s) + block_multiplier * G * K) +
...
#pragma unroll
      for (int i = 0; i < K; ++i) {
        *(grad_offset_softmax) =
            cache_g_mask_before_softmax[i] - p_mask_shm[i] * sum;
        grad_offset_softmax += 1;
      }
    }
  }
...
void _dcnv4_col2im_cuda(cudaStream_t stream,
    ...
    switch (K) {
      case 9:
        if (check_backward_warpp(d_stride, D)) {
          ...
          kernel =
              backward_kernel_dcn<scalar_t, d_stride, stride_type, 1, 9, true>;
        }
        break;
      case 8:
        if (check_backward_warpp(d_stride, D)) {
          ...
          kernel =
              backward_kernel_dcn<scalar_t, d_stride, stride_type, 1, 8, true>;
        }
        break;
      default:
        printf("K=%ld\n", K);
    ...
    switch (K) {
      case 9:
        if (check_backward_warpp(d_stride, D)) {
          ...
          kernel =
              backward_kernel_dcn<scalar_t, d_stride, stride_type, 1, 9, false>;
        }
        break;
      case 8:
        if (check_backward_warpp(d_stride, D)) {
          ...
          kernel =
              backward_kernel_dcn<scalar_t, d_stride, stride_type, 1, 8, false>;
        }
        break;
      default:
        printf("K=%ld\n", K);
  ...
  if (err != cudaSuccess) {
    ...
    printf(
        ...
        num_threads.z, shm_size);
    AT_ASSERTM(false, "kernel launch error");
  }
}

template <typename scalar_t>
void dcnv4_col2im_cuda(cudaStream_t stream,
                       const scalar_t* value,        // B, H * W, (G * D)
                       const scalar_t* p_offset,     // B, H * W, (G*K*3)
                       const scalar_t* grad_output,  // B, H_out*W_out, G * D
                       const int kernel_h,
    ...
    switch (d_stride) {
      case 1:
        _dcnv4_col2im_cuda<scalar_t, scalar_t, 1>(
            ...
            grad_im, grad_offset, block_thread, softmax, padded_offset_dim);
        break;
      case 2:

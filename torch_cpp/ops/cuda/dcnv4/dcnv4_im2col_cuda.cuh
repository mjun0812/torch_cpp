...
template <typename scalar_t,
          ...
          int K,
          bool softmax>
__global__ void forward_kernel_dcn(const scalar_t* p_value,
...
template <typename scalar_t,
          ...
          int K,
          bool softmax>
__global__ void forward_kernel_dcn_reg(const scalar_t* p_value,
                                       const scalar_t* p_offset,
                                       scalar_t* p_output,
                                       const int G,
                                       const int D,
...
void _dcnv4_im2col_cuda(cudaStream_t stream,
  ...
  if (softmax) {
    ...
    switch (K) {
      case 9:
        kernel = forward_kernel_dcn_reg<scalar_t, d_stride, stride_type, 1, 9,
                                        false>;
        break;
      case 8:
        kernel = forward_kernel_dcn_reg<scalar_t, d_stride, stride_type, 1, 8,
                                        false>;
        break;
      default:
        printf("K=%ld\n", K);

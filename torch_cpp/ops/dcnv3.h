...
#include "cpu/dcnv3/dcnv3_cpu.h"

#ifdef WITH_CUDA
#include "cuda/dcnv3/dcnv3_cuda.h"
#endif
...
#ifdef WITH_CUDA
    return dcnv3_cuda_forward(input, offset, mask, kernel_h, kernel_w, stride_h,
...
std::vector<at::Tensor> dcnv3_backward(
  ...
  if (input.type().is_cuda()) {
    ...
    return dcnv3_cuda_backward(input, offset, mask, kernel_h, kernel_w,
                               ...
                               grad_output, im2col_step, remove_center);
#else
    AT_ERROR("Not compiled with GPU support");
#endif
  }
